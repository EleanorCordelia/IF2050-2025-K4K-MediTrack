...
public class PenggunaDAO {
    ...
    // 1. Ambil semua baris dari tabel pengguna
    ...
    public boolean insertPengguna(Pengguna p) {
        String sql = "INSERT INTO pengguna(nama, email, password, tanggalLahir, jenisKelamin, tinggiBadan, beratBadan) " +
                "VALUES(?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = SQLiteConnection.getConnection();
    ...
    public boolean updatePengguna(Pengguna p) {
        String sql = "UPDATE pengguna SET nama=?, email=?, password=?, tanggalLahir=?, jenisKelamin=?, tinggiBadan=?, beratBadan=? " +
                "WHERE idPengguna=?";
        try (Connection conn = SQLiteConnection.getConnection();
